...
It is not possible to compute from Monte Carlo simulations the free energy of a system, since it is not a derivative of the partition function. However, we can compute its derivative with respect to the system. This is useful to compute the Casimir force {\textcolor{red}{This is the total thermodynamic force - not the Casimir force !The extraction of the Casimir force is not discussed for either of the methods - you need to complete this}} 
\begin{align}
...
\end{align}
where the prefactor $\frac{1}{2}$ take into account the prefactor between Ising and SOS models in Hamiltonian \eqref{energie-sos-ising}. By doing the ?? {\color{red} Table of values } , we notice that every term in the sum is equal to $-1$ independently of $k$, since contrary to Ising models, SOS models do not possess any bulk energy. We {\textcolor{red}{You need to explain how you extract the Casimir force from this by looking at two different L values}}
