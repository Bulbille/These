...
{\color{red}
Critical systems have a macroscopic correlation length, which leads to finite-size effects in the free energy. Following De Gennes and Fisher steps \cite{gambassi_casimir_2009}, we transpose the Casimir effect into critical systems. We consider a system of $N$ spins.
}
%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%
{\color{red} for coherence, changed notations from $\ell$ to$L$. I didn't find any reference to compare these results, do you have any ?}


Here we consider the case 
\begin{equation}
V(h) = \begin{cases} 0 \ {\rm for }\ 0 \less h \less 0\ \\
\infty \ {\rm otherwise} \end{cases}
\end{equation}
This corresponds to a one dimensional elastic line confined between two impenetrable walls separated by a distance $L$. The Hamiltonian $\hat H$ is that for a quantum well of width $L$, whose solutions are \cite{claude_cohen-tannoudji_mecanique_2018}
\begin{equation}
...
%%%%%%%%%%%%%%%%%%%%
{\color{red} I've renamed $P_0$ as $\mu$, which is a chemical potential, and your $\mu$ as $\ell$, since it's a length scale.}
In the above well known example we confine the surface and then compute the pressure. This is an example of the constant volume ensemble. Physically we could also consider the case of a system which is confined softly by an externally imposed pressure $\mu$ the constant pressure ensemble. In this case the potential is given by
\begin{equation}
V(h) = \begin{cases} \mu h \ {\rm for }\ h \greater 0\ \\
\infty \ {\rm for}\ h\leq 0 \end{cases}
\end{equation}
The time independent Schr\"odinger equation for the eigenfunctions here is
\begin{equation}
-\frac{1}{2\sigma\beta}\frac{d^2 \psi_n(h)}{dh^2} + \mu\beta h \psi_n(h) = \epsilon_n\psi_n(h).
\end{equation}
...
\begin{equation}
\frac{1}{2\sigma\beta\ell^2}\frac{d^2 \psi_n(z')}{dz'^2}- \ell\beta \mu(z'-\varepsilon_n)\psi_n(z')=0,
\end{equation}
where $\varepsilon_n= \epsilon_n/(\ell\beta\mu)$. Now we chose $\ell$ so that
\begin{equation}
2\sigma\beta^2\mu \ell^3=1,
\end{equation}
and we see that 
\begin{equation}
\ell = \left(\frac{1}{2\beta^2\sigma \mu}\right)^{\frac{1}{3}},
\end{equation}
...
\label{psi-n-airy}
\end{equation}
{\color{red} BC are $\psi_n'(0)=0$, which makes $\varepsilon_n=\alpha_n$ with $\alpha_0=1.0187$ the first zero of $Ai'$, instead of 2.3337. }
The boundary condition $\psi_n'(h) = 0$ then show that we must choose $\varepsilon_n=\alpha_{n}$. This means that the ground state energy is
\begin{equation}
\epsilon_0 = \alpha_0\mu\beta \ell= \frac{\alpha_0\mu\beta}{(2\sigma\beta^2\mu)^\frac{1}{3}}= \frac{\alpha_0 \mu^\frac{2}{3}\beta^\frac{1}{3}}{2^\frac{1}{3} \sigma^\frac{1}{3}},
\end{equation}
and where we note that $\alpha_0 = 1.0187$. The free energy is
\begin{equation}
f= \frac{\alpha_0 \mu^\frac{2}{3}}{2^\frac{1}{3} \sigma^\frac{1}{3}\beta^\frac{2}{3}}
\end{equation}
From the original partition function we see that $h$ is conjugate to $\mu$ and so we find the average height is given by
\begin{equation}
\bar h= < h> = \frac{\partial f}{\partial \mu} = \frac{2}{3}\frac{\alpha_0 }{2^\frac{1}{3} \sigma^\frac{1}{3}\beta^\frac{2}{3}\mu^\frac{1}{3}} = \frac{2}{3}\alpha_0 {\color{red} \ell}
\label{h1}
\end{equation}
and solving for $\mu$ in terms of $\overline h$ gives
\begin{equation}
\mu = \frac{4}{27}\frac{\alpha_0^3 T^2}{\sigma \overline h^3}
\end{equation}
we see that $\mu$ behaves exactly in the same way as the pressure of a confined elastic line
...
\begin{equation}
\xi = \frac{2^\frac{1}{3}(\sigma T)^\frac{1}{3}}{(\alpha_2-\alpha_0)\mu^\frac{2}{3}}.
\end{equation}
When written in terms of $\overline h$ the above correlation length behaves in the same way
as for the free elastic line, however when $\mu$ is fixed we see that the behavior as a function 
...
The probability density function for the height of the interface at a single point is given by
\begin{equation}
p_1(h) = \frac{{\rm Ai}^2(\frac{h}{\mu}-\alpha_0)}{\int_0^\infty dh' {\rm Ai}^2(\frac{h'}{\mu}-\alpha_0)}
\label{p-airy}
\end{equation}
However if we write the height variable in terms of the length scale $\mu$, $h(x)= \ell z(x)$ we find that $z$ has the single point probability density function
\begin{equation}
...
\begin{figure}
\centering
\includegraphics[width=0.7\linewidth]{int-dyn/etats-laser.pdf}
\caption{Solutions $\psi_n^2$ from Eq \eqref{psi-n-airy}, with the probability density function in the thermodynamic limit being $p(z) = \psi_0^2(z)$.}
\label{etats-laser}
\end{figure}

Using this we find the average height is given by
\begin{equation}
< h> = \overline h= \ell z_0,
\end{equation}
where 
\begin{equation}
...
{\color{red} For $L$ large, we look for the flat interface solution \eqref{def-theta}}, so we look for an eigenvalue such that $L \theta \sim 1$. We write
\begin{equation}
\phi = L \theta
\end{equation}
...
\theta = \frac{(2n+1)\pi}{L}
\end{equation}
{\color{purple}However we notice that this approximation is only valid if $L(1-r) \gg1$. For large $\beta$ this approximation is simply equivalent to $L\gg1$, however when $\beta$ is small it requires
that $H\beta \gg1$.}
{\color{red} I don't get this argument, why $L(1-r) \gg1$ ?}
...
\label{ground-sos}
\end{equation}
{\color{red} The limit $L\to \infty$ is in agreement with \cite{guyer_sine-gordon_1979}.}

\subsubsection*{First excited state eigenvector}
In order to compute the second eigenvalue $\lambda_1$ we look for an odd an antisymmetric solution with $c=-1$. We thus find
\begin{equation}
\exp(i L\theta) = \frac{r-\exp(-i\theta)}{r-\exp(i\theta)}
\label{theta}
\end{equation}
...
\label{excited-sos}
\end{equation}
{\color{red}In Fig \ref{large-l-limit}, we show the agreement between the computation of the first two eigenvectors with respect to the exact diagonalization of the transfer matrix, in the large system limit. }

\begin{figure}
\centering
\includegraphics[width=0.7\linewidth]{int-dyn/null_deanJ.pdf}
\caption{$\lambda_0$ and $\lambda_1$ in the large $L$ limit from the transfer matrix, compared to analytical approximation $L\to \infty$ with q \eqref{ground-sos} and Eq \eqref{excited-sos} with respect to size $L$, for $J=1$ and $\beta=1$. In black, the largest eigenvalue of an infinite system \cite{guyer_sine-gordon_1979}.}
\label{large-l-limit} 
\end{figure}
...
\end{equation}
{\color{red}
In Fig \ref{fig-high-temp}, we show the agreement between the transfer matrix and Eq \eqref{high-temperature}. We see that in the domain of definition of the logarithm, the high-temperature approximation holds.
}
\begin{figure}
\centering
