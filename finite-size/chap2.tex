...
%%%%%%%%%%%%%%%%%%%%
{\color{red} can we rename $P_0$ as $\mu$, since it's the chemical potential ? And in the paragraph then, rename $\mu$ as another thing ?}
...
\end{align}
with the generalized magnetization $M^\ast = \sum_i V(\sigma_i)$, which gives us a suitable upper limit of integration if we want to get the Casimir force.
...
\begin{align}
    h^\ast = < \sum_i V(h_i) >
\end{align}
quation \eqref{lopes-gen} writes as
\begin{align}
